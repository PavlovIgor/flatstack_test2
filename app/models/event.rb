class Event < ApplicationRecord
  ...
  scope :dow, ->(date) {
    where('date <= ? AND extract(dow from date) in (?)', date, date.wday)
  }
  scope :dom, ->(date) {
    where('date <= ? AND extract(day from date) in (?)', date, date.mday)
  }
  scope :doy, ->(date) {
    where('date <= ? AND extract(doy from date) in (?)', date, date.yday)
  }

  validates :name, :date, :repeat, presence: true

  enum repeat: [:not_repeat, :weekly, :monthly, :yearly]
